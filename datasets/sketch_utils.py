...
def create_random_pose(rotation=None, elevation=None, return_angles=False):
    ...
    eye = np.array([
        np.sin(np.radians(rotation)) * np.cos(np.radians(elevation)),
        np.sin(np.radians(elevation)),
        np.cos(np.radians(rotation)) * np.cos(np.radians(elevation))
    ]) * 2.5
    target = np.zeros(3)
    
    camera_pose = np.array(pyrr.Matrix44.look_at(
        eye=eye, 
        target=target, 
...
def get_P_from_transform_matrix(matrix):
    ...
    return np.array([
        [112 / t, 0, width / 2],
        [0, 112 / t, width / 2],
        [0, 0, 1]]) @ np.concatenate([
            rotation,
            np.expand_dims(-1 * rotation @ location, axis=1)
        ], axis=1)
