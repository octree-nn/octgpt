...
class BinarySphericalQuantizer(nn.Module):
  def __init__(self, embed_dim, beta, gamma0, gamma, zeta,
    ...
    assert persample_entropy_compute in [
        'group', 'analytical'], "persample_entropy_compute must be either 'group' or 'analytical'"
    assert cb_entropy_compute in [
        'group', 'nce'], "cb_entropy_compute must be either 'group' or 'nce'"
    ...
    # soft_entropy: Sec 3.2 of https://arxiv.org/pdf/1911.05894.pdf
  ...
  def soft_entropy_loss(self, z):
    ...
    distance = - 2 * \
        torch.einsum('... g c, d c ->... g d', divided_z, group_code_book)
  ...
  def get_hard_per_sample_entropy(self, zb_by_sample):
    ...
    persample_entropy = - probs_per_dim * \
        torch.log(probs_per_dim + 1e-8) - (1 - probs_per_dim) * \
